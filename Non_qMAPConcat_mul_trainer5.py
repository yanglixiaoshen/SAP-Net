...
def run(train_batch_size, epochs, lr, weight_decay, config, exp_id, log_dir,
    ...
    if os.path.exists(os.path.join(args.log_dir_IQA6, "state.pkl")):
        model.load_state_dict(torch.load(os.path.join(args.log_dir_IQA6, "state.pkl")), strict=False) #
        logger.info("Successfully loaded pretrained IQA_model.")


    optimizer = Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    if os.path.exists(os.path.join(args.log_dir_IQA6, "optimizer_state.pkl")):
        optimizer.load_state_dict(torch.load(os.path.join(args.log_dir_IQA6, "optimizer_state.pkl")))
    ...
    for epoch in range(epochs)[1:]:
        ...
        for batch_num, (im_mw, imp_iwt, gt_iwt, im_dmos) in enumerate(train_loader):
            ...
            plt.imsave(os.path.join(args.log_dir_IQA6, 'hr.jpg'), gt_iwt.detach().cpu().numpy()[0][0])
            plt.imsave(os.path.join(args.log_dir_IQA6, 'sr.jpg'), pre_iwt.detach().cpu().numpy()[0][0])
            plt.imsave(os.path.join(args.log_dir_IQA6, 'lr.jpg'), imp_iwt.detach().cpu().numpy()[0][0])

            loss_batch.backward()
            optimizer.step()

            torch.save(model.state_dict(), os.path.join(args.log_dir_IQA6, "state.pkl"))
            torch.save(optimizer.state_dict(), os.path.join(args.log_dir_IQA6, "optimizer_state.pkl"))
        ...
        if epoch % 1 == 0:
            torch.save(model.state_dict(), os.path.join(args.log_dir_IQA6, "state.pkl.epoch{}".format(epoch)))
...
if __name__ == "__main__":
    ...
    ensure_dir(args.log_dir_IQA6)

    log_dir = '{}/{}'.format(args.log_dir_IQA6, 'tf')

    ensure_dir(log_dir)

    shutil.copy2(__file__, os.path.join(args.log_dir_IQA6, "script.py"))  # copy2:复制文件和状态到后一个文件  print(__file__)   打印所执行文件当前的位置路径
    ...
    fh = logging.FileHandler(os.path.join(args.log_dir_IQA6, "log_train.txt"))
