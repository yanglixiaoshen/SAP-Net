...
if __name__ == "__main__":
    ...
    logger = logging.getLogger("test-IQA_NonCBAM1")
    logger.setLevel(logging.DEBUG)
    logger.handlers = []
    ch = logging.StreamHandler()
    logger.addHandler(ch)
    fh = logging.FileHandler(os.path.join(args.log_dir_IQA3, "log_test_e444_i153.txt"))
    ...
    with torch.no_grad():
        ...
        if os.path.exists(os.path.join(args.log_dir_IQA3, "state.pkl.epoch153")):
            model.load_state_dict(torch.load(os.path.join(args.log_dir_IQA3, "state.pkl.epoch153")), strict=False)  #
            # logger.info("Successfully loaded pretrained IQA_model.")
        ...
        np.savetxt(os.path.join(args.log_dir_IQA3, 'testgt_score_i153.txt'), score_save)
        np.savetxt(os.path.join(args.log_dir_IQA3, 'test_score_i153.txt'), test_score)
