...
def run(train_batch_size, epochs, lr, weight_decay, config, exp_id, log_dir,
    ...
    if os.path.exists(os.path.join(args.log_dir_MW, "state.pkl.epoch")):
        model.load_state_dict(torch.load(os.path.join(args.log_dir_MW, "state.pkl.epoch")),  strict=False)
        logger.info("Successfully loaded pretrained MW_model.")

    optimizer = Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    if os.path.exists(os.path.join(args.log_dir_MW, "optimizer_state.pkl")):
        optimizer.load_state_dict(torch.load(os.path.join(args.log_dir_MW, "optimizer_state.pkl")))
    ...
    for epoch in range(epochs):
        ...
        for batch_num, (imp_dwt, gt_dwt, imp_iwt, gt_iwt) in enumerate(train_loader):
            ...
            torch.save(model.state_dict(), os.path.join(args.log_dir_MW, "state.pkl"))
            torch.save(optimizer.state_dict(), os.path.join(args.log_dir_MW, "optimizer_state.pkl"))
            logger.info("[EPOCH{}:ITER{}] <LOSS>={:.4} <TRAIN_PSNR>={:.4}".format(epoch, iter, loss_batch.item(), psnr_batch))
        ...
        if epoch % 2 == 0:
            torch.save(model.state_dict(), os.path.join(args.log_dir_MW, "state.pkl.epoch{}".format(epoch)))
...
if __name__ == "__main__":
    ...
    ensure_dir(args.log_dir_MW)

    log_dir = '{}/{}'.format(args.log_dir_MW, 'tf')

    ensure_dir(log_dir)

    if not os.path.exists(args.log_dir_MW):
        os.makedirs(args.log_dir_MW)
    shutil.copy2(__file__, os.path.join(args.log_dir_MW, "script.py"))  # copy2:复制文件和状态到后一个文件  print(__file__)   打印所执行文件当前的位置路径

    logger = logging.getLogger("train-ODIEhance_MW")
    logger.setLevel(logging.DEBUG)
    logger.handlers = []
    ch = logging.StreamHandler()
    logger.addHandler(ch)
    fh = logging.FileHandler(os.path.join(args.log_dir_MW, "log_train.txt"))
