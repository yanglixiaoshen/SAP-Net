...
from model.Non_qMAPConcat_mul_IQA import Model
...
def run(train_batch_size, epochs, lr, weight_decay, config, exp_id, log_dir,
    ...
    if os.path.exists(os.path.join(args.log_dir_IQA4, "state.pkl")):
        model.load_state_dict(torch.load(os.path.join(args.log_dir_IQA4, "state.pkl")), strict=False) #
        logger.info("Successfully loaded pretrained IQA_model.")


    optimizer = Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    if os.path.exists(os.path.join(args.log_dir_IQA4, "optimizer_state.pkl")):
        optimizer.load_state_dict(torch.load(os.path.join(args.log_dir_IQA4, "optimizer_state.pkl")))
    ...
    for epoch in range(epochs)[1:]:
        ...
        for batch_num, (im_mw, imp_iwt, gt_iwt, im_dmos) in enumerate(train_loader):
            ...
            plt.imsave(os.path.join(args.log_dir_IQA4, 'hr.jpg'), gt_iwt.detach().cpu().numpy()[0][0])
            plt.imsave(os.path.join(args.log_dir_IQA4, 'sr.jpg'), pre_iwt.detach().cpu().numpy()[0][0])
            plt.imsave(os.path.join(args.log_dir_IQA4, 'lr.jpg'), imp_iwt.detach().cpu().numpy()[0][0])

            loss_batch.backward()
            optimizer.step()

            torch.save(model.state_dict(), os.path.join(args.log_dir_IQA4, "state.pkl"))
            torch.save(optimizer.state_dict(), os.path.join(args.log_dir_IQA4, "optimizer_state.pkl"))
        ...
        if epoch % 2 == 0:
            torch.save(model.state_dict(), os.path.join(args.log_dir_IQA4, "state.pkl.epoch{}".format(epoch)))
...
if __name__ == "__main__":
    ...
    logger = logging.getLogger("test-IQA_NonCBAM143")
    logger.setLevel(logging.DEBUG)
    logger.handlers = []
    ch = logging.StreamHandler()
    logger.addHandler(ch)
    fh = logging.FileHandler(os.path.join(args.log_dir_IQA6, "log_test_e444_i60.txt"))
    ...
    with torch.no_grad():
        ...
        if os.path.exists(os.path.join(args.log_dir_IQA6, "state.pkl.epoch60")):
            model.load_state_dict(torch.load(os.path.join(args.log_dir_IQA6, "state.pkl.epoch60")), strict=False)  #
            # logger.info("Successfully loaded pretrained IQA_model.")

        # test_list = np.loadtxt(os.path.join(args.log_dir_IQA, 'test_score.txt'))
        # val_list = np.loadtxt(os.path.join(args.log_dir_IQA, 'validate_score.txt'))
        # test_list = np.hstack((val_list, test_list))
        # np.savetxt(os.path.join(args.log_dir_IQA, 'test_val_score.txt'), test_list)
        test_list = np.loadtxt(os.path.join(args.log_dir_IQA3, 'test_val_score.txt'))
        ...
        for i in range(test_list.shape[0]):
            ...
            patches_mw, patches_imp = NonOverlappingCropPatches_1(imp)
            scores = []
            for j in range(len(patches_imp)):
                ...
                pre_score = model(imp_iwt, pre_iwt - imp_iwt)
                scores.append(pre_score.squeeze(0).detach().cpu())
            ...
            print('The {} img {} imp score and gt --- {:.4f} {:.4f} '.format(i, imp_name, imp_score, gt_dmos[int(imp_id)]))
            logger.info('{:.4f} {:.4f} '.format(imp_score,gt_dmos[int(imp_id)]))
        ...
        score_save = np.hstack((test_score, gt_score))
        np.savetxt(os.path.join(args.log_dir_IQA6, 'testgt_score_i60.txt'), score_save)
        np.savetxt(os.path.join(args.log_dir_IQA6, 'test_score_i60.txt'), test_score)
        srocc, krocc, plcc, rmse, mae = metricIQA(test_score, gt_score)

        print("Validation-Test Results- <PLCC>: {:.4f} <KROCC>: {:.4f} <SROCC>: {:.4f} <RMSE>: {:.7f} <MAE>: {:.7f}"
              .format(plcc, krocc, srocc, rmse, mae))
        logger.info("Validation-Test Results- <PLCC>: {:.4f} <KROCC>: {:.4f} <SROCC>: {:.4f} <RMSE>: {:.7f} <MAE>: {:.7f}"
              .format(plcc, krocc, srocc, rmse, mae))
