...
if __name__ == "__main__":
    ...
    fh = logging.FileHandler(os.path.join(args.log_dir_IQA3, "log_test_e444_i143.txt"))
    ...
    with torch.no_grad():
        ...
        if os.path.exists(os.path.join(args.log_dir_IQA3, "state.pkl.epoch143")):
            model.load_state_dict(torch.load(os.path.join(args.log_dir_IQA3, "state.pkl.epoch143")), strict=False)  #
            # logger.info("Successfully loaded pretrained IQA_model.")
        ...
        np.savetxt(os.path.join(args.log_dir_IQA3, 'testgt_score_i143.txt'), score_save)
        np.savetxt(os.path.join(args.log_dir_IQA3, 'test_score_i143.txt'), test_score)
