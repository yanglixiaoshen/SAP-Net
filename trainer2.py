...
# train: ehance.epoch316. batch_size = 12 P=9 lr=5e-4 validate: ehance.epoch444, 418; iqa.epoch64, 60;
...
if __name__ == "__main__":
    ...
    ensure_dir(args.log_dir_IQA)

    log_dir = '{}/{}'.format(args.log_dir_IQA, 'tf')

    ensure_dir(log_dir)

    shutil.copy2(__file__, os.path.join(args.log_dir_IQA, "script.py"))  # copy2:复制文件和状态到后一个文件  print(__file__)   打印所执行文件当前的位置路径
    ...
    fh = logging.FileHandler(os.path.join(args.log_dir_IQA, "log_train.txt"))
