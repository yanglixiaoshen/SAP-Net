...
if __name__ == "__main__":
    ...
    logger = logging.getLogger("test-IQA2")
    logger.setLevel(logging.DEBUG)
    logger.handlers = []
    ch = logging.StreamHandler()
    logger.addHandler(ch)
    fh = logging.FileHandler(os.path.join(args.log_dir_IQA2, "log_test_e444_i99.txt"))
    ...
    with torch.no_grad():
        ...
        if os.path.exists(os.path.join(args.log_dir_IQA2, "state.pkl.epoch99")):
            model.load_state_dict(torch.load(os.path.join(args.log_dir_IQA2, "state.pkl.epoch99")), strict=False)  #
            # logger.info("Successfully loaded pretrained IQA_model.")
        ...
        test_list = np.loadtxt(os.path.join(args.log_dir_IQA, 'test_val_score.txt'))


        gt_dmos = np.loadtxt(config['yl360Dataset1']['impDMOS_reg'])
        test_score = []
        gt_score = []
        ...
        for i in range(test_list.shape[0]):
            imp_id = test_list[i]
            imp_name = np.loadtxt(config['yl360Dataset1']['refimpID_pth'], dtype=str)[int(imp_id), 1]

            imp = default_loader('.'.join([os.path.join(config['yl360Dataset1']['img_ref_IMG_pth'], imp_name), 'jpg']))
        ...
        np.savetxt(os.path.join(args.log_dir_IQA2, 'testgt_score.txt'), score_save)
        np.savetxt(os.path.join(args.log_dir_IQA2, 'predicted_score.txt'), test_score)
