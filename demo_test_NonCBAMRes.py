...
from model.Non_CBAM_ResCBAMIQA import Model
...
if __name__ == "__main__":
    ...
    fh = logging.FileHandler(os.path.join(args.log_dir_IQA5, "log_test_e444_i56.txt"))
    ...
    with torch.no_grad():
        ...
        if os.path.exists(os.path.join(args.log_dir_IQA5, "state.pkl.epoch56")):
            model.load_state_dict(torch.load(os.path.join(args.log_dir_IQA5, "state.pkl.epoch56")), strict=False)  #
            # logger.info("Successfully loaded pretrained IQA_model.")
        ...
        np.savetxt(os.path.join(args.log_dir_IQA5, 'testgt_score_i56.txt'), score_save)
        np.savetxt(os.path.join(args.log_dir_IQA5, 'test_score_i56.txt'), test_score)
